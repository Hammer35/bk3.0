...
Python package redis фиксируется отдельно в lock-файле и должен быть совместим с Celery 5.6, Python 3.12 и Valkey 8.1.x; совместимость подтверждается интеграционными тестами.
...
# 35. Bootstrap pins фактического каркаса

На 23 сентября 2026 года каркас проекта зафиксирован на конкретных версиях:
...
- Psycopg 3.3.6
- Uvicorn 0.53.0
- PostgreSQL Docker image 17.10-bookworm
- Valkey Docker image 8.1.10-alpine
- HTMX 2.0.10
- Alpine.js 3.17.4
- Tailwind CSS 4.3.3

Эти версии являются bootstrap baseline (базовой точкой каркаса).

Правило:

> Не обновлять их «по пути» при разработке функций. Любое изменение версии проходит отдельный compatibility review и тесты.
