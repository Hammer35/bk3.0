...
- **каждый Pin перед публикацией должен быть выбран/подтверждён пользователем**, если у нас нет отдельного письменного разрешения Pinterest на иной flow.

Наш Telegram / MAX Approval Flow становится не просто UX-функцией, а важной частью compliance.

Режим «AI сам одобряет и публикует всё выше Quality Score» конфликтует с текущей формулировкой Developer Guidelines и не должен включаться в production без явного письменного разрешения Pinterest.
...
- показать каждый Pin;
- пользователь явно выбирает каждый объект;
- после этого можно одной кнопкой подтвердить публикацию выбранного набора.

То есть bulk submit допустим как UI-механика после individual selection, но нельзя проектировать «автоматически одобрить всё».

Нужно подтвердить этот UX с Pinterest при Standard access review.
...
Но окончательная публикация Pin требует user selection/approval, пока Pinterest письменно не разрешит иное.
...
3. Допустим ли multi-select, если пользователь просмотрел каждый Pin?
