...
- metadata;
- generation source;
- результаты проверок соответствия требованиям с замечаниями и основаниями (не прогноз продвижения Pinterest);
- created_at.

Approval относится к конкретной PinVersion и отдельному действию пользователя. Новая версия требует нового отдельного одобрения.
...
SYSTEM_AUTOPILOT не является допустимым каналом одобрения. Для одобренного состояния обязательны конкретный пользователь, версия пина и отдельное действие подтверждения. Разрешения Pinterest не заменяют одобрение пользователя.
